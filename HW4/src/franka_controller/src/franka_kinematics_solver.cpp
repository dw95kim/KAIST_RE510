...
public:
	...
	void TargetEEPoseCallback(const geometry_msgs::PoseStampedConstPtr& msg){
        ...
        KDL::JntArray q_out = KDL::JntArray(7);
        ...
        int IKerror = KDLInverseKinematics(franka_chain_, target_frame, q_current, q_out);
        if(IKerror == 1)
        {
            printf("IK Error");
        }
        ...
        for(unsigned int i=0; i<7;i++){
            desired_joint_state.name[i] = current_joint_state.name[i];
            desired_joint_state.velocity[i] = current_joint_state.velocity[i];
            desired_joint_state.effort[i] = current_joint_state.effort[i];
        }
	...
	int KDLInverseKinematics(const KDL::Chain robot_chain, const KDL::Frame target_frame,
        ...
        KDL::ChainIkSolverPos_NR_JL iksolver_pos_nr_jl(franka_chain_,q_min,q_max,fksolver,iksolver_vel,50,1e-2);//Maximum 100 iterations, stop at accuracy 1e-3
    ...
    int init(){
        ...
        // // Chain Creation
        bool ret0 = franka_kdl_tree.getChain("panda_link0","panda_link7",franka_chain_);
