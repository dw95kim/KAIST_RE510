...
double tl = 0.1; //translational limit
double rl = 20.0*(M_PI/180); //rotational limit

virtual_master_device::master_dev_state master_state_;

geometry_msgs::TwistStamped master_st_pos_;
geometry_msgs::TwistStamped master_st_vel_;
std_msgs::Bool master_st_bt_;
...
//        ROS_ERROR("workspace limit");
        ...
        master_state_.pos.twist.linear.x = feedback->pose.position.x;
        master_state_.pos.twist.linear.y = feedback->pose.position.y;
        master_state_.pos.twist.linear.z = feedback->pose.position.z;

        master_state_.pos.twist.angular.x = rolling;
        master_state_.pos.twist.angular.y = pitching;
        master_state_.pos.twist.angular.z = yawing;
